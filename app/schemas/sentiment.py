...
class SentimentResponse(BaseModel):
    ...
    confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confianza en la prediccion (0-1)"
    )
    ...
    model_version: str = Field(
        ...,
        description="Version del modelo usado"
    )

    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),  # lambda = funcion anonima que se ejecuta cada vez
...
class BatchSentimentResponse(BaseModel):
    ...
    texts_analyzed: int = Field(
        ...,
        description="Cantidad de textos analizados"
    )
